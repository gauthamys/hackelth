    ...
    render(){
        return(
            ...
            <button className={'rounded-full ml-5 clickable text-white w-8 h-8 opacity-80 '+(this.state.open ? 'bg-green-600': 'bg-green-400')} onClick={this.handleClick}>{this.state.open ? 'x' : 'i'}</button>
                <div className={'relative shadow-xl ml-5 mt-3 w-80 h-80 opacity-90 rounded-xl bg-green-500 transform transition duration-1000 ease-in-out '+(this.state.open ? 'visible':'hidden')}>
